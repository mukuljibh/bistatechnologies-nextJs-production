...
function NavElements({ label, submenu, directPath }: navProps) {
    ...
    return (
        <div className="lg:mt-0 mt-5 lg:px-3 lg:py-6 hover:text-black group " onClick={() => setMobileToggle(prev => !prev)}>
            <div className="flex justify-between">
                {submenu ? <h1 className="cursor-pointer ">{label}</h1> : <Link onClick={() => setToggle(false)} href={directPath || '#'}>{label}</Link>}
            ...
            {
                submenu &&
                <SubNavElement submenu={submenu} mobileToggle={mobileToggle} />
            }
